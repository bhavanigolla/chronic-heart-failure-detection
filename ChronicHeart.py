        ...
        for root, dirs, directory in os.walk(filename):
            for j in range(len(directory)):
                ...
                if '.dat' in directory[j]:
                    ...
                    #mfccs = librosa.feature.mfcc(y=signals, sr=fields['fs'], n_mfcc=13)
...
def runML():
    ...
    ml_model = RandomForestClassifier(n_estimators=15, random_state=0,criterion='entropy')
...
font = ('times', 14, 'bold')
title = Label(main, text='Chronic heart failure detection for a Secure Life')
...
mlButton = Button(main, text="Run ML model", command=runML)
mlButton.place(x=280,y=150)
mlButton.config(font=font1) 

dlButton = Button(main, text="Run DL Model ", command=runDL)
dlButton.place(x=450,y=150)
